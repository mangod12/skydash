  ...
  return (
    <div className="h-full flex">
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        {active ? (
          <>
            <MissionHeader mission={active} />
            <TabBar tab={tab} onTab={setTab} />
            <div className="flex-1 min-h-0 overflow-y-auto p-4">
              <AnimatePresence mode="wait">
                <motion.div key={tab} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.2 }}>
                  {tab === 'entities' && <EntitiesTab mission={active} />}
                  {tab === 'notes' && <NotesTab mission={active} />}
                  {tab === 'briefing' && <BriefingTab mission={active} />}
                  {tab === 'map' && <MapTab mission={active} />}
                  {tab === 'timeline' && <TimelineTab mission={active} />}
                </motion.div>
              </AnimatePresence>
            </div>
          </>
...
function MissionList({ missions, activeId, onSelect, loading }) {
